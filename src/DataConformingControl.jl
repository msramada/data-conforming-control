...
export  Standard_LQR_SDP, Prob5, Prob6
...
function Prob5(A, B, Q, R, W, V, X, γ_prime)
    N = size(X)[2]
    Σ_data = 1/(N) * X * X'
    model = Model(Mosek.Optimizer);
    rₓ = size(A)[1]
    ...
    L, Σ = value.(L), value.(Σ)
    K_prob5 = L * inv(Σ)
    println("Lyupanov condition error: (L∞ norm) is ", maximum(abs.(Σ-W-B*V*B'-(A+B*K_prob5)*Σ*(A+B*K_prob5)')))
    return K_prob5, Σ
end

function Prob6(A, B, Q, R, W, V, X, U, γ)
    N = size(X)[2]
    rₓ = size(A)[1]
    rᵤ = size(B)[2]
    D = [X;U]
    ...
    L, Σ = value.(L), value.(Σ)
    K_prob6 = L * inv(Σ)
    println("Lyupanov condition error: (L∞ norm) is ", maximum(abs.(Σ-W-B*V*B'-(A+B*K_prob6)*Σ*(A+B*K_prob6)')))
